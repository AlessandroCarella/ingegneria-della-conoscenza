...
def outputExplain():
    print(
        ...
        "+===================================+===========================+\n",
        "|  feature name(0 (not liked song)) | probability feature (val) |\n",
        "+-----------------------------------+---------------------------+\n",
        "|   feature name(1 (liked song))    | probability feature (val) |\n",
        "+-----------------------------------+---------------------------+",
    )
    prPurple(
        ...
        if "N" == result or result == "n":
            ...
            if isSpotifyTrackUrl(result):
                ...
                if trackAudioFeatures:
                    evidence = trackAudioFeaturesToQueryEvidence(
                        trackAudioFeatures, explicit)
                    queryOutput(data, differentialColumn, evidence)
                else:
                    print(
                        "Couldn't retreive audio features for the given song from spotify api")
            else:
